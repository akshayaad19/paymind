...
**Delivery problems become conversations:** if a paid purchase order doesn't arrive, the customer clicks **Not received** and a **case opens on the order's payment**, so shop and customer can talk it through in the case (the shop ships again or refunds); when the customer later clicks **Delivered**, the case closes. For a free replacement, after the photos are approved the customer **confirms the delivery address**, the shop ships it there with the tracking ID (*"let us know when you receive it or if you face any issues"*), and the customer answers **✅ Received** (closes the case) or **❌ Not received** (the case goes back to the shop, which can ship again).
